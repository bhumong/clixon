 ...
 * @see yang2cli_treeref_encode
 */
int
mtpoint_encode(cbuf       *cb,
               const char *delim,
               const char *domain,
               const char *spec)
{
    cprintf(cb, "%s", MTPOINT_PREFIX);
    cprintf(cb, "%s%s", delim, domain);
    cprintf(cb, "%s%s", delim, spec);
    return 0;
}

/*! Decode mount-point using domain and spec
 *
 * Expected format:
 * mtpoint:<domain>:<spec>
 * @param[in]  str     Decoded string
 * @param[in]  delim   Delimiter string that may not occur in any of the elements
 * @param[out] domain  Domain name, free after use
 * @param[out] spec    yspec name, free after use
 * @retval     0       OK
 * @retval    -1       Error
 * @see yang2cli_treeref_decode
 */
int
mtpoint_decode(const char *str,
               const char *delim,
               char      **domain,
               char      **spec)
{
    ...
    if (delim == NULL){
        clixon_err(OE_YANG, EINVAL, "delim is NULL");
        goto done;
    }
    if (str == NULL)
        goto ok;
    if ((vec = clixon_strsep1(str, delim, &nvec)) == NULL)
        goto done;
    if (nvec != 3)
        goto ok;
    if (strcmp(vec[0], MTPOINT_PREFIX) != 0)
        goto ok;
    if (domain){
        if ((*domain = strdup(vec[1])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
    if (spec){
        if ((*spec = strdup(vec[2])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
...
/*! Helper function to add mount-point reference
 */
static int
gen_mtpoint_add(cbuf      *cb,
                yang_stmt *ys)
{
    int        retval = -1;
    yang_stmt *yspec;
    ...
    if ((yspec = ys_spec(ys)) != NULL &&
        ...
        ydomain = ys_domain(ys);
        cprintf(cb, ",\"");
        if (mtpoint_encode(cb, ":", yang_argument_get(ydomain), yang_argument_get(yspec)) < 0)
            goto done;
        cprintf(cb, "\"");
    }
    retval = 0;
...
/*! Encode yang reference info into clispec treeref
 *
 * Format(with "-" as delim)
 *   <domain>-<spec>-<module>[@<revision>]-<keyword>-<argument>
 * Used:
 * (1) By yang2cli uses/grouping as pare-tree reference, (see yang2cli_uses):
 *               @domain--spec--example--2025-12-01--grouping--pg1
 *     See: yang2cli_uses, yang2cli_grouping_wrap, controller_yang2cli_wrap:
 * (2) By backend_cache code to get cache file name
 *               domain--spec--example--2025-12-01--grouping--pg1.cli
 *     See cache_autocli_read_gen_write
 *     XXX: use digest
 * @param[out] cb       CLIgen buf
 * @param[in]  delim    Delimiter string that may not occur in any of the elements (except the last)
 * @param[in]  domain   Domain name
 ...
 * @param[in]  tag      Keyword, eg "grouping"
 ...
 */
int
yang2cli_treeref_encode(cbuf       *cb,
                        const char *delim,
                        const char *domain,
                        const char *spec,
                        const char *module,
                        ...
                        const char *argument)
{
    if (domain == NULL || spec == NULL || module == NULL || keyword == NULL || argument == NULL){
        clixon_err(OE_YANG, EINVAL, "domain, spec, module, keyword or argument is NULL");
        return -1;
    }
    cprintf(cb, "%s", domain);
    cprintf(cb, "%s%s", delim, spec);
    cprintf(cb, "%s%s", delim, module);
    cprintf(cb, "%s%s", delim, revision?revision:"");
    cprintf(cb, "%s%s", delim, keyword);
    cprintf(cb, "%s%s", delim, argument);
    return 0;
}

/*! Decode clispec treeref reference into yang info
 *
 * Format(with "-" as delim)
 *   <domain>-<spec>-<module>[@<revision>]-<keyword>-<argument>
 ...
 */
int
yang2cli_treeref_decode(const char *str,
                        const char *delim,
                        char      **domain,
                        char      **spec,
                        char      **module,
                        ...
                        char      **argument)
{
    ...
    if (str == NULL || delim == NULL){
        clixon_err(OE_YANG, EINVAL, "str or delim is NULL");
        goto done;
    }
    if ((vec = clixon_strsep1(str, delim, &nvec)) == NULL)
        goto done;
    if (nvec != 6)
        goto ok;
    if (domain){
        if ((*domain = strdup(vec[0])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
    if (spec){
        if ((*spec = strdup(vec[1])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
    if (module){
        if ((*module = strdup(vec[2])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
    if (revision){
        if ((*revision = strdup(vec[3])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
    if (keyword){
        if ((*keyword = strdup(vec[4])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
    if (argument){
        if ((*argument = strdup(vec[5])) == NULL){
            clixon_err(OE_UNIX, errno, "strdup");
            goto done;
        }
    }
 ...
 * @retval     0         OK
 * @retval    -1         Error
 * @see yang2cli_leaf_var_sub  Its sub-function
 */
static int
 ...
 * @retval     0       OK
 * @retval    -1       Error
 * @see yang2cli_leaf_var_sub   which is the main function
...
/*! Generate callback, labels and terminate clispec string
 */
static int
gen_callback(clixon_handle h,
             yang_stmt    *ys,
             int           key_leaf,
             cbuf         *cb)
{
...
/*! Generate CLI code for Yang leafref statement
 *
 * @param[in]  h         Clixon handle
 ...
 * @param[in]  cvv     Cvec with array of range_min/range_max cv:s (if YANG_OPTIONS_RANGE is set in options)
 * @param[in]  patterns Cvec of regexp patterns
 * @param[in]  fraction_digits for decimal64, how many digits after period
 * @param[out] cb        Buffer where cligen code is written
 * @retval     1         OK
 * @retval     0         Skip
 * @retval    -1         Error
 */
static int
yang2cli_leafref_var(clixon_handle h,
                     yang_stmt    *ys,
                     yang_stmt    *yreferred,
                     char         *helptext,
                     int           options,
                  ...
                  int           key_leaf)
{
    ...
    if (strcmp(restype, "empty") == 0){
        if (callback){
            if (gen_callback(h, ys, key_leaf, cb) < 0)
                goto done;
        }
        goto skip;
    }
    ...
    if (strcmp(restype, "union") == 0){
        ...
        if ((ret = yang2cli_leafref_var(h, ys, yreferred, helptext,
                                        options, cvv, patterns, fraction_digits,
                                        cvtype,
                                        yrestype, cvtypestr, regular_value,
                                        callback, key_leaf,
                                        cb)) < 0)
            goto done;
        if (ret == 0)
              ...
              cbuf         *cb)
{
    ...
    /* Called a second time in yang2cli_leaf_var, room for optimization */
              ...
              cbuf         *cb)
{
    ...
    if (nodeid_split(yang_argument_get(ys), &prefix, &argument) < 0)
        goto done;
 ...
 * XXX See also yang2cli_grouping() in apps/cli/cli_generate.c
 */
int
