...
/*! Collect clixon mem statistics
 *
 * @param[in]     h         Clixon handle
 * @param[in]     modules   If 1, also include per-module stats, otherwise only global and datastore stats
 * @param[in]     xml_type  XML stats type
 * @param[in,out] cbret     Cligen buf, with XML content appended
 * @retval        0         OK
 * @retval       -1         Error
  */
int
clixon_backend_stats(clixon_handle  h,
                     int            modules,
                     xml_stats_enum xml_type,
                     cbuf          *cbret)
{
